...
def report_splice_mates(args, results, group_list, sample_records):
    ...
    for group in group_list:
        ...
        totals = {sample_ids[i]:base_val for (i,base_val) in enumerate(base_vals)}
        #go off the base sample_id list which is a subset of the junction sample_ids
        for sample_id in results['all_sample_sums'][group].keys():
            if sample_id in totals:
                totals[sample_id] += results['all_sample_sums'][group][sample_id]
