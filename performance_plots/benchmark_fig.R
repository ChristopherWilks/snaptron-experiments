...
ggplot(m, aes(x=Query.Size, y=Query.Time, color=Type, linetype=Constrained)) + labs(x='Query size (KB)', y='Time (seconds)') + geom_line() + geom_point() + theme_bw() + theme(legend.title=element_blank()) + scale_y_continuous(limits=c(0, 17))
dev.off()
